...
before producing a computation or module value:
...
This form keeps imports, parameters, definitions, and the resulting module value in one context-forming block.
...
A library usually ends in a module value; an executable ends in its main computation.
...
such as passing a packed value directly.
...
`let` retains the defining equation, which suits type aliases, module signatures, and local abbreviations.
...
New code uses `Thk` and `Ret`, the names supplied by the Builtin packed value's surface.
...
| Type fields in packed values | `UpperCamel` | `= Bool`, `= Option` |
...
## Named Components and Packed Values
...
Named fields follow the naming convention of their payload: type components use `UpperCamel`,
and value components use `lower_snake_case`.
Place an abstract type component before the values whose classifiers mention it.
A manifest witness records a type equation as part of the interface;
an abstract witness publishes its kind and identity without disclosing that equation.
...
Library interfaces are easiest to use when their components are named.
...
Ordinary term projections traverse transparent named products and stop at an unopened abstract witness.
At an abstract witness boundary, use one projection-pattern group to open the existential telescope once
...
A final ordinary pattern such as `builtin` retains the complete packed value for forwarding,
while the preceding projections introduce only the requested local names.
Omit that alias when the consumer does not forward the packed value.
This projection-pattern idiom serves the role of a module `use` without adding a separate binding form.

The canonical Builtin contract is the single source of `@[builtin(...)]` host-capability metadata.
...
by importing the Builtin contract and selecting the required fields from the supplied value,
...
and are re-exported through manifest components whose fields carry the public type names.
