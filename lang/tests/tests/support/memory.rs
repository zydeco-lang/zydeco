...
pub fn source(body: &str) -> String {
    ...
    format!(
        r#"
let (/Uninit; /Init; /Fields; /Field; /DynamicField; /runtime_fields; /Ptr; /fields; /records; /headers; /arrays; /views; /Slice; /slices; /Storage; /DynamicStorage; /Codec; /DynamicCodec; /storage; /codecs; /fixed; /dynamic; /allocation; /pointer; /raw) =
