...
impl ScalarMemory {
        ...
        unsafe {
            match literal {
                | Literal::Integer(integer) => match integer {
                    ...
                    | IntegerLiteral::Int(value) => address.write(&value.to_le_bytes()),
                    ...
                    | IntegerLiteral::UInt(value) => address.write(&value.to_le_bytes()),
    ...
    pub(crate) fn load(
        ...
        let width = match primitive {
            | PrimitiveType::Integer(integer) => integer.storage_bits() / 8,
        ...
        let value = Self::literal(primitive, bytes).ok_or(RuntimeError::IntegerRange)?.into();
    ...
    fn literal(primitive: PrimitiveType, bytes: &[u8]) -> Option<Literal> {
        macro_rules! integer {
            ($variant:ident, $type:ty) => {
                Literal::Integer(
                    IntegerLiteral::$variant(<$type>::from_le_bytes(bytes.try_into().ok()?))
                        .with_type(IntegerType::$variant)?,
                )
            };
        }
