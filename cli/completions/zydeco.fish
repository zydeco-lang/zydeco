...
complete -c zydeco -n "__fish_zydeco_needs_command" -f -a "test" -d 'Run explicitly selected test packages with empty stdin'
...
complete -c zydeco -n "__fish_zydeco_using_subcommand help; and not __fish_seen_subcommand_from show passes __doc-example-worker doc fmt run check test repl build help" -f -a "test" -d 'Run explicitly selected test packages with empty stdin'
