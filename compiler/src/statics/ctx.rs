...
impl<Ann: Clone> Ctx<Ann> {
    ...
    pub fn extend(&mut self, other: impl IntoIterator<Item = (VVar<Ann>, TValue<Ann>)>) {
        self.vmap.extend(other);
    }
    pub fn lookup(&self, x: &VVar<Ann>) -> Option<&TValue<Ann>> {
        self.vmap.get(x)
    }
    pub fn decl(&mut self, d: &Declare<Ann>) -> Result<(), NameResolveError<Ann>> {
        match d {
            ...
            Declare::Codata { name, dtors, ann } => {
                ...
                for (dtor, args, ret) in dtors {
                    self.dtors
                        .insert(dtor.clone(), (name.clone(), args.clone(), ret.clone()))
                        .map_or(Ok(()), |_| {
                            Err(NameResolveError::DuplicateDeclaration {
