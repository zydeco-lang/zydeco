...
impl TypeCheck for Span<TermComputation> {
    ...
    fn syn_step(
        ...
        Ok(match self.inner_ref() {
            ...
            TermComputation::Force(Force(v)) => {
                match v.syn(ctx.clone())?.synty {
                    SynType::TypeApp(ty_app) => {
                        ...
                        let kd =
                            self.span().make(ty_body.to_owned()).syn(ctx)?;
                ...
                match ty_scrut.synty {
                    SynType::TypeApp(ty_app) => {
                        ...
                        bool_test(
                            ...
                            || {
                                self.span().make(InconsistentMatchers {
                                    unexpected,
                                    missing,
                                })
                            },
                        ...
                        for ty in &ty_arms {
                            if let Some(ty_opt) = &ty_opt {
                                ty_opt.eqv(ty, ctx.clone(), || {
                                    self.span().make(InconsistentBranches(
                                        ty_arms.clone(),
                                    ))
                                })?;
                            } else {
            ...
            TermComputation::Dtor(Dtor { body, dtor, args }) => {
                match body.syn(ctx.clone())?.synty {
                    SynType::TypeApp(ty_app) => {
                        let tvar = ty_app.tvar;
    ...
    fn ana_step(
        ...
        Ok(match self.inner_ref() {
            TermComputation::Ret(Ret(v)) => match &typ.synty {
                SynType::TypeApp(ty_app) => {
                ...
                match &typ.synty {
                    SynType::TypeApp(ty_app) => {
                        ...
                        bool_test(
                            unexpected.is_empty() && missing.is_empty(),
                            || {
                                self.span().make(InconsistentCoMatchers {
                                    unexpected,
                                    missing,
                                })
                            },
